...
module.exports = function (sequelize, DataTypes) {
  ...
  Revision.saveNoteRevision = function (note, callback, publish) {
    Revision.findAll({
      where: {
        noteId: note.id
      },
      ...
      if (revisions.length <= 0) {
        ...
        const extracted = sequelize.models.Note.extractMeta(note.content)
        let tag = null;
        if (extracted.meta.cop) {
          tag = (extracted.meta.major_version || "1") + "." + (extracted.meta.minor_version || "0") + "." + (extracted.meta.patch_version || "0") ;
          if (!publish) {
            tag = `${tag}-stage`
          }
        }
  ...
  Revision.getLatestRevision = function (noteId, publish, callback) {
    const tagCondition = publish ? {
      [Op.and]: [
        { [Op.ne]: null },
        { [Op.notLike]: '%-stage' }
      ]
