...
module.exports = function (sequelize, DataTypes) {
            ...
            if (Note.checkFileExist(filePath)) {
              ...
              if (fsModifiedTime.isAfter(dbModifiedTime) && note.content !== body) {
                note.update({
                  ...
                  //   // 更新或创建 cop 记录
                  // }
                  sequelize.models.CopTreeV2.createOrUpdate({
                    cop: extracted.meta.cop,
                    noteId: note.id,
                    noteShortId: note.shortid,
                    noteAlias: note.alias,
                    fileId: extracted.meta.file_id || "",
                    parentFileId: extracted.meta.parent_file_id || "",
                    majorVersion: extracted.meta.major_version || 1,
                    minorVersion: extracted.meta.minor_version || 0,
                    patchVersion: extracted.meta.patch_version || 0,
  ...
  Note.parseNoteInfo = function (body) {
    ...
    if (parsed.meta.cop) {
      ret['cop'] = parsed.meta.cop
      ret['file_id'] = parsed.meta.file_id
      ret['parent_file_id'] = parsed.meta.parent_file_id
      ret['major_version'] = parsed.meta.major_version
      ret['minor_version'] = parsed.meta.minor_version
      ret['patch_version'] = parsed.meta.patch_version
    }
    return ret
  }
  ...
  Note.generateWebTitle = function (title) {
    title = !title || title === 'Untitled' ? 'HedgeDoc - Collaborative markdown notes' : title
    return title
  }
  ...
  Note.generateCopNote = function (content, meta) {
    return content.replace(/patch_version:\s*\d+/i, `patch_version: ${meta.patch_version}`)
  }
