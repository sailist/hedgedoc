  ...
  models.Note.parseNoteId(id, function (err, _id) {
    ...
    models.Note.findOne({
      ...
      if (!note.alias && note.id !== id) {
        note.update({
          alias: models.Note.encodeNoteId(note.id)
        })
        logger.info(`update alias ${note.id} to ${models.Note.encodeNoteId(note.id)}`)
      }
...
exports.newNote = async function (req, res, body) {
  ...
  if (noteId) {
    try {
      ...
      if (note) {
        const title = models.Note.parseNoteTitle(body)
        await note.update({
          content: body,
          title: title
        })
        const extract = models.Note.extractMeta(body)
        await models.CopTreeV2.createOrUpdate({
          noteId: note.id,
          cop: extract.meta.cop,
        ...
        history.updateHistory(req.user.id, note.alias, body, Date.now())
  ...
  models.Note.create({
    ...
    const extract = models.Note.extractMeta(body)
    const alias = models.Note.encodeNoteId(note.id);
    await note.update({
      alias: alias
    })
    history.updateHistory(req.user.id, alias, body, Date.now())
    ...
    logger.debug("createOrUpdate CopTree")
