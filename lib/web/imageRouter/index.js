...
imageRouter.post('/uploadimage', function (req, res) {
  ...
  const form = formidable({
    ...
    filename: function (name, ext, part) {
      ...
      // 使用文件大小、时间戳和用户ID生成唯一文件名
      const timestamp = part.headers["last-modified"] || Date.now();
      const fileSize = part.headers["content-length"] || 0;
      const userId = req.user ? req.user.id : 'anonymous';
      const uniqueString = `${timestamp}-${fileSize}-${userId}`;
  ...
  form.parse(req, async function (err, fields, files) {
    if (err) {
      ...
      // 检查文件是否已存在
      try {
        const existingUrl = await uploadProvider.checkFileExists(files.image.filepath)
        if (existingUrl) {
          rimraf.sync(tmpDir)
          logger.debug(`File already exists, returning existing URL: ${existingUrl}`)
          return res.send({
            link: existingUrl
          })
        }
