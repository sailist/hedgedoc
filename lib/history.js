...
function updateHistory (userid, aliasId, document, time) {
  if (userid && aliasId && typeof document !== 'undefined') {
    getHistory(userid, function (err, history) {
      if (err || !history) return
      if (!history[aliasId]) {
        history[aliasId] = {}
      }
      const noteHistory = history[aliasId]
      const noteInfo = models.Note.parseNoteInfo(document)
      noteHistory.id = aliasId
      // noteHistory.text = noteInfo.title
      noteHistory.time = time || Date.now()
      // noteHistory.tags = noteInfo.tags
      Object.assign(noteHistory, noteInfo)
